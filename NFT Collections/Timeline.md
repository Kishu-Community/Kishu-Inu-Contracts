# NFT Collection Order And Context

This file keeps the NFT collection archive in historical order as closely as current public records and recovered community notes allow.

## Verified On-Chain Order

1. Kishu Crate ETH
   - Contract: [`0x7ba9a9d77ac6ddcb4d8b63665ca985a2fec6430f`](https://etherscan.io/token/0x7ba9a9d77ac6ddcb4d8b63665ca985a2fec6430f)
   - Creation block: [`13489134`](https://etherscan.io/block/13489134)
   - Creation transaction: [`0xb230eadb255176fd8be88b6a95069536126e82a0bad6e818617ee3ddc325820a`](https://etherscan.io/tx/0xb230eadb255176fd8be88b6a95069536126e82a0bad6e818617ee3ddc325820a)
   - Deployer/current owner checked: [`0x90eAd86FCa54eE9a1FE1C55C0ACE5896f4319802`](https://etherscan.io/address/0x90eAd86FCa54eE9a1FE1C55C0ACE5896f4319802)

2. Kishu Takeover ETH
   - Contract: [`0x8a240d66f09a3a1fc8f108adbce5a5ba61702ced`](https://etherscan.io/token/0x8a240d66f09a3a1fc8f108adbce5a5ba61702ced)
   - Creation block: [`13495309`](https://etherscan.io/block/13495309)
   - Creation transaction: [`0xe47221897e4969172e17190b34ca73ef92098f84e70a0edbf4f34c225d299480`](https://etherscan.io/tx/0xe47221897e4969172e17190b34ca73ef92098f84e70a0edbf4f34c225d299480)
   - Deployer: [`0x3141FCef2fCf8ede0470fE2B5afb497a0F70102d`](https://etherscan.io/address/0x3141FCef2fCf8ede0470fE2B5afb497a0F70102d)
   - Current owner checked: [`0x99834733C91aAE2f5BB0725105c9E843Cb297A27`](https://etherscan.io/address/0x99834733C91aAE2f5BB0725105c9E843Cb297A27)

3. Kishu Knights ETH
   - Contract: [`0x3f07e5d9fdbdb4baf018eb466faa1754d882572e`](https://etherscan.io/token/0x3f07e5d9fdbdb4baf018eb466faa1754d882572e)
   - Creation block: [`13498615`](https://etherscan.io/block/13498615)
   - Creation transaction: [`0xe673fada3b3646a4e2262d40d7518403c8ccebb36c9527dc4e1a8160af2bcb2d`](https://etherscan.io/tx/0xe673fada3b3646a4e2262d40d7518403c8ccebb36c9527dc4e1a8160af2bcb2d)
   - Deployer/current owner checked: [`0x629CeCc4cBfbf5984139B6afB68c1dbaC82Bc4D4`](https://etherscan.io/address/0x629CeCc4cBfbf5984139B6afB68c1dbaC82Bc4D4)

4. Kishuverse ETH
   - Contract: [`0x0be5dd8dc316a936942b1f58667c9ea1ebb71e07`](https://etherscan.io/token/0x0be5dd8dc316a936942b1f58667c9ea1ebb71e07)
   - Creation block: [`13532364`](https://etherscan.io/block/13532364)
   - Creation transaction: [`0x715189797475a7320ab1a6f4f4919e475075fb6fecf570d2fe5da75e1569c8d6`](https://etherscan.io/tx/0x715189797475a7320ab1a6f4f4919e475075fb6fecf570d2fe5da75e1569c8d6)
   - Deployer/current owner checked: [`0xc38568F9C9080de06fA924c4353F8616aa502c1f`](https://etherscan.io/address/0xc38568F9C9080de06fA924c4353F8616aa502c1f)

5. Kishu Kingdom Avatars Polygon
   - Contract: [`0xad411f5a8ba84533b350a2e50a46ba7511edfcae`](https://polygonscan.com/token/0xad411f5a8ba84533b350a2e50a46ba7511edfcae)
   - Current owner checked: [`0x35e45f9DA89143F5aB6ea705784beB65482875fA`](https://polygonscan.com/address/0x35e45f9DA89143F5aB6ea705784beB65482875fA)
   - Creation transaction: not yet recorded in this archive.

## Community Context To Verify

- Kishu Takeover and Kishu Knights were part of, or closely associated with, the original Kishu Crate release context based on recovered community notes.
- Kishu Takeover is identified in recovered community notes as presented at the time as created or owned by the team member known as `Captain Kishu`.
- Kishu Knights is identified in recovered community notes as presented at the time as created or owned by the team member known as `CML`.
- Kishuverse appears later in the public on-chain order and is documented separately because its contract is a standalone ERC-721 minting contract.
- Kishuverse is identified in recovered community notes as presented at the time as created or owned by the team members known as `Robinhood` and `Mo Kishu`.
- Kishu Kingdom Avatars should be treated as a separate later NFT/project-era artifact unless current public records prove an active game, contract, or deployment path.

These notes are intentionally separated from the verified on-chain facts above.
